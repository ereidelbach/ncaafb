...
def scrape_player_statistics_offense(min_year, max_year):
    ...
    if min_year == max_year:
        filename_pass = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_off_pass.csv' % 
                (min_year))
    else:
        filename_pass = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_off_pass.csv' % 
                (min_year, max_year))
    ...
    if min_year == max_year:
        filename_run = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_off_run.csv' % 
                (min_year))
    else:
        filename_run = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_off_run.csv' % 
                (min_year, max_year))
    ...
    if min_year == max_year:
        filename_rec = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_off_rec.csv' % 
                (min_year))
    else:
        filename_rec = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_off_rec.csv' % 
                (min_year, max_year))
...
def scrape_player_statistics_defense(min_year, max_year):
    ...
    if min_year == max_year:
        filename_def_tackles = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_def_tackles.csv' % 
                (min_year))
    else:
        filename_def_tackles = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_def_tackles.csv' % 
                    (min_year, max_year))
    ...
    if min_year == max_year:
        filename_def_int = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_def_int.csv' % 
                (min_year))
    else:
        filename_def_int = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_def_int.csv' % 
                    (min_year, max_year))
    ...
    if min_year == max_year:
        filename_def_pbu = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_def_pbu.csv' % 
                (min_year))
    else:
        filename_def_pbu = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_def_pbu.csv' % 
                    (min_year, max_year))
    ...
    if min_year == max_year:
        filename_def_pass = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_def_pass.csv' % 
                (min_year))
    else:
        filename_def_pass = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_def_pass.csv' % 
                    (min_year, max_year))
    ...
    if min_year == max_year:
        filename_def_fumb = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_def_fumb.csv' % 
                (min_year))
    else:
        filename_def_fumb = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_def_fumb.csv' % 
                    (min_year, max_year))
...
def scrape_player_statistics_kicks(min_year, max_year):
    ...
    if min_year == max_year:
        filename_xp = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_kick_xp.csv' 
                % (min_year))
    else:
        filename_xp = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_kick_xp.csv' % 
                (min_year, max_year))
    ...
    if min_year == max_year:
        filename_fg = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_kick_fg.csv' 
                % (min_year))
    else:
        filename_fg = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_kick_fg.csv' % 
                (min_year, max_year))
    ...
    if min_year == max_year:
        filename_kicking = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_kicking.csv' % 
                (min_year))
    else:
        filename_kicking = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_kicking.csv' % 
                (min_year, max_year))
...
def scrape_player_statistics_returns(min_year, max_year):
    ...
    if min_year == max_year:
        filename = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_ret_kick.csv' 
                % (min_year))
    else:
        filename = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_ret_kick.csv' % 
                (min_year, max_year))
    ...
    if min_year == max_year:
        filename = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_ret_punt.csv' 
                % (min_year))
    else:
        filename = (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%i_to_%i_ret_punt.csv' % 
                (min_year, max_year))
...
def scrape_team_statistics(min_year, max_year):
    ...
    '''    
    #--------------------------------------------------------------------------
    # Scrape the following team stats:
    #    - Passing Info            - Turnovers
    #    - Rushing Inf             - Sacks / TFLs
    #--------------------------------------------------------------------------
    # sack data is not available before 2005 so adjust accordingly
    if min_year < 2005:
        ...
        url = ('https://www.sports-reference.com/cfb/play-index/sgl_finder.cgi?' + 
               f'request=1&match=game&year_min={min_year}&year_max={max_year}' + 
               '&c1stat=pass_yds&c1comp=gt&c1val=0&c2stat=rush_att&c2comp=gt' + 
               '&c2val=0&c3stat=pass_int&c3comp=gt&c3val=0&c4val=0' + 
               '&order_by=date_game&order_by_asc=Y')
        
        # Rename various column headers
        headers = ['School', 'Date', 'G', 'Loc', 'Opponent', 'WL', 'Pass Comp', 
                   'Pass Att', 'Comp Pct', 'Pass Yard', 'Pass TD', 'Rush Att', 
                   'Rush Yard', 'Rush Avg', 'Rush TD', 'Fum', 'Pass Int', 'Turnovers']
        ...
        url = ('https://www.sports-reference.com/cfb/play-index/sgl_finder.cgi?' + 
               f'request=1&match=game&year_min={min_year}&year_max={max_year}' + 
               '&c1stat=pass_yds&c1comp=gt&c1val=0&c2stat=rush_att&c2comp=gt' + 
               '&c2val=0&c3stat=pass_int&c3comp=gt&c3val=0&c4stat=tackles_loss' + 
               '&c4comp=gt&c4val=0&order_by=date_game&order_by_asc=Y')
        
        # Rename various column headers
        headers = ['School', 'Date', 'G', 'Loc', 'Opponent', 'WL', 'Pass Comp', 
                   'Pass Att', 'Comp Pct', 'Pass Yard', 'Pass TD', 'Rush Att', 
                   'Rush Yard', 'Rush Avg', 'Rush TD', 'TFL', 'Sk', 'Fum', 
                   'Pass Int', 'Turnovers']
    
    # set the filename
    filename = ''
    if min_year == max_year:
        filename = 'data_scraped/ncaa_team/sub_stats/ncaa_team_%i.csv' % (min_year)
    else:
        filename = ('data_scraped/ncaa_team/sub_stats/ncaa_team_%i_to_%i.csv' % 
                    (min_year, max_year))
    
    # scrape data and write to disk
    print('###------- Scraping NCAA Team Game Statistics' + 
          f' from {min_year} to {max_year} -------###')
    run_scraper(filename, headers, url, team = True)

    #--------------------------------------------------------------------------
    # Scrape the following team stats:
    #    - Points Scored                 - Opp. Rushing Info
    #    - Opp. Passing Info             - Opp. Tunovers
    #--------------------------------------------------------------------------
    # set the desired url for scraping purposes using the input date range
    url = ('https://www.sports-reference.com/cfb/play-index/sgl_finder.cgi?' + 
           f'request=1&match=game&year_min={min_year}&year_max={max_year}' + 
           '&c1stat=points&c1comp=gt&c1val=0&c2stat=opp_pass_att&c2comp=gt' + 
           '&c2val=0&c3stat=opp_rush_att&c3comp=gt&c3val=0&c4stat=opp_turnovers' + 
           '&c4comp=gt&c4val=0&order_by=date_game&order_by_asc=Y')

    # Rename various column headers
    headers = ['School', 'Date', 'G', 'Loc', 'Opponent', 'WL', 'Total TD', 
               'Points For', 'Points Difference', 'Pass Comp opp', 
               'Pass Att opp', 'Comp Pct opp', 'Pass Yard opp', 'Pass TD opp', 
               'Rush Att opp', 'Rush Yard opp', 'Rush Avg opp', 'Rush TD opp', 
               'Fum opp', 'Pass Int opp', 'Turnovers opp']
    
    # set the filename
    filename = ''
    if min_year == max_year:
        filename = 'data_scraped/ncaa_team/sub_stats/ncaa_team_opp_%i.csv' % (min_year)
    else:
        filename = ('data_scraped/ncaa_team/sub_stats/ncaa_team_opp_%i_to_%i.csv' % 
                    (min_year, max_year))
        
    # scrape data and write to disk
    print('###------- Scraping NCAA Opponent Game Statistics (Part 1)' + 
          f' from {min_year} to {max_year} -------###')
    run_scraper(filename, headers, url, team = True)
    ...
    url = ('https://www.sports-reference.com/cfb/play-index/sgl_finder.cgi?' + 
           f'request=1&match=game&year_min={min_year}&year_max={max_year}' + 
           '&c1stat=opp_td&c1comp=gt&c1val=0&c2val=0&c3val=0&c4val=0' + 
           '&order_by=date_game&order_by_asc=Y')

    # Rename various column headers
    headers = ['School', 'Date', 'G', 'Loc', 'Opponent', 'WL', 'Total TD opp', 
               'Points Against']
    
    # set the filename
    filename = ''
    if min_year == max_year:
        filename = 'data_scraped/ncaa_team/sub_stats/ncaa_team_oppV2_%i.csv' % (min_year)
    else:
        filename = ('data_scraped/ncaa_team/sub_stats/ncaa_team_oppV2_%i_to_%i.csv' % 
                    (min_year, max_year))
        
    # scrape data and write to disk
    print('###------- Scraping NCAA Opponent Game Statistics (Part 2)' + 
          f' from {min_year} to {max_year} -------###')
    run_scraper(filename, headers, url, team = True)
    
    # format the scraped team stats
    format_team_statistics(min_year, max_year)
    
    return

def format_team_statistics(min_year, max_year):
    '''
    ...
    ------
        min_year : int
            Starting year for which data was scraped
        max_year : int
            Ending year for which data was scraped
    ...
    ''' 
    # determine what files to look for based on what was just written
    if min_year == max_year:
        files = glob.glob(f'data_scraped/ncaa_team/sub_stats/*_{max_year}.csv')
    else:
        files = glob.glob(f'data_scraped/ncaa_team/sub_stats/*{min_year}_{max_year}.csv')
    
    # read all files for the specified date range into a dataframe
    df_all = pd.DataFrame()
    for file in files:
        ...
        if len(df_all) == 0:
            ...
            df_all = pd.merge(df_all, df_file, how = 'left', 
                              on = ['School', 'Date', 'G', 'Loc', 'Opponent', 'WL'])
    
    # insert team codes for each team and their respective opponent
    df_all['Team Code'] = set_team_code(df_all['School'].tolist())
    df_all['Team Code opp'] = set_team_code(df_all['Opponent'].tolist())    
    
    # standardize team names for the player's team
    df_all = rename_school(df_all, 'School')
    # standardize team names for the player's opponent
    df_all = rename_school(df_all, 'Opponent')
    
    # set the desired order of the columns
    list_column_order = ['School', 'Team Code', 'Date', 'G', 'Loc', 'Opponent', 
                     'Team Code opp', 'WL', 'Points For', 'Points Against', 
                     'Points Difference', 'Pass Comp', 'Pass Att', 'Comp Pct', 
                     'Pass Yard', 'Pass TD', 'Pass Int', 'Rush Att', 'Rush Yard', 
                     'Rush Avg', 'Rush TD', 'TFL', 'Sk', 'Fum', 'Turnovers', 
                     'Total TD', 'Pass Comp opp', 'Pass Att opp', 'Comp Pct opp', 
                     'Pass Yard opp', 'Pass TD opp', 'Rush Att opp', 
                     'Rush Yard opp', 'Rush Avg opp', 'Rush TD opp', 'Fum opp', 
                     'Pass Int opp', 'Turnovers opp', 'Total TD opp']
    # ignore sacks and TFLs before 2005
    if min_year < 2005:
        list_column_order.remove('Sk')
        list_column_order.remove('TFL')
    # set desired order of columns
    df_all = df_all[list_column_order] 
    
    # set the name of the output file
    if min_year == max_year:
        filename = f'data_scraped/ncaa_team/ncaa_team_data_{min_year}.csv'
    else:
        filename = f'data_scraped/ncaa_team/ncaa_team_data_{min_year}_{max_year}.csv'
...
def merge_files_for_year(year):
    ...
    for file in sorted(glob.glob(
            'data_scraped/ncaa_player/sub_stats/*%s*.csv' % str(year))):
        # ignore any existing master file
        if file == (
                'data_scraped/ncaa_player/sub_stats/ncaa_player_%s.csv' % 
                str(year)):
            continue
    ...
    # insert team codes for each team and their respective opponent
    df_master['Team Code'] = set_team_code(df_master['School'].tolist())
    df_master['Team Code opp'] = set_team_code(df_master['Opponent'].tolist())    
    ...
    # output to disk        
    df_master.to_csv('data_scraped/ncaa_player/ncaa_player_%i.csv' % (year), 
                     index = False)
...
#    scrape_team_statistics(year, year)
#    format_team_statistics(year, year)