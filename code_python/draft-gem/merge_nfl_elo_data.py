...
:DESCRIPTION: Ingests newly created NFL elo data and formats it in the desired 
...
dict_variables_to_fix = {
        ...
        'WR': ['recElo', 'ypgElo', 'ypcElo', 'tgtElo', 'yptElo', 'tdElo', 
               'eloC', 'date', 'opp'],
        'DEF': ['tacklesElo', 'tflElo', 'sackElo', 'qbhElo', 'intElo', 'pbuElo', 
                'ffElo', 'date', 'opp']
        }
...
def merge_elo_data():
    ...
    for pos in tqdm.tqdm(['QB', 'RB', 'WR', 'DEF']):
        ...
        df = pd.read_csv('data_raw/nfl_%s.csv' % pos, 
    ...
    #   - all pos-related variables ('xxx_Pos')
    ...
    for column in df_master.columns:
        ...
        elif not any(substring in column for substring in ['_w', '_l', 
                                                           '_last', '_Pos']):
            list_cols_keep.append(column)
    ...
    # set all count values of type int with missings set to 0
    for column in [col for col in df_master.columns if '_count' in col]:
        list_row = []
        for row in df_master[column]:
            if row == '':
                list_row.append(0)
            else:
                list_row.append(row)
        df_master[column] = list_row
    ...
    for index, row in tqdm.tqdm(df_master.iterrows()):
        for position in ['QB', 'RB', 'WR', 'DEF']:
            ...
            if (((row[position + '_count'] == '') or (row[position + '_count'] == 0)) or
                ((row[position + '_count'] <= 7) and (row['last_year'] != True))):
    ...
    df_meta = df_meta[['id_nfl', 'pos_nfl', 'pos_nfl_std', 'to_nfl']]
    df_meta = df_meta.rename(columns = {'id_nfl':'unique_id',
                                        'pos_nfl':'position',
                                        'pos_nfl_std':'position_std',
                                        'to_nfl':'To'})
    ...
    # export csv to disk
    df_merged.to_csv('data_elo/nfl_elo.csv', index = False)
    ...
    with open('data_elo/nfl_elo.json', 'wt') as out:
