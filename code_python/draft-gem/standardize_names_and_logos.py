...
#            'data_team/teams_%s.csv' % league))
    df_team_names = pd.read_csv('data_team/teams_%s.csv' % league)
...
def rename_teams(list_teams, league, std_name = 'Team'):
    ...
    Purpose: Rename a list of teams to standardized name as specified in 
        the file 'data_team/teams_nfl.csv' (for NFL teams) or 
        'data_team/teams_ncaa.csv' for (NCAA teams)
...
def standardize_logo_ncaa(df):
    ...
    df_pictures = pd.read_csv('data-team/teams_ncaa.csv')
...
def standardize_logo_nfl(df):
    ...
    '''    
    # ingest the school names and URLs from a flat file    
    df_pictures = pd.read_csv('data-team/teams_nfl.csv')
